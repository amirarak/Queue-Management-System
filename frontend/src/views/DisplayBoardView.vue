      ...
      <div class="current-section">
        ...
        <div v-if="queueStore.currentTicket" class="current-ticket">
          ...
          <div class="ticket-number">
            {{ queueStore.currentTicket.ticketCode || queueStore.currentTicket.ticketNumber }}
          </div>
        </div>
        ...
        <div v-if="queueStore.calledTickets.length > 0" class="recently-called">
          ...
          <div class="recent-list">
            <div v-for="ticket in queueStore.calledTickets.slice(0, 3)" :key="ticket.id" class="recent-item">
              <span class="recent-number">{{ ticket.ticketCode || ticket.ticketNumber }}</span>
        ...
        <div v-if="queueStore.waitingTickets.length > 0" class="queue-list">
          ...
          >
            <div class="item-header">
              <span class="item-number">{{ ticket.ticketCode || ticket.ticketNumber }}</span>
...
.time-display { font-size: 48px; font-weight: 700; }
.date-display { font-size: 18px; color: rgba(255,255,255,0.7); }
...
.current-ticket { background: var(--color-accent); border-radius: var(--border-radius-lg); padding: 60px 40px; text-align: center; box-shadow: 0 10px 40px rgba(220,38,38,0.3); }
.ticket-label { font-size: 24px; margin-bottom: 20px; opacity: 0.9; }
.ticket-number { font-size: clamp(60px, 10vw, 120px); font-weight: 700; line-height: 1; letter-spacing: -2px; }
...
.recent-number { font-size: 28px; font-weight: 700; letter-spacing: -1px; }
...
.item-number { font-size: 40px; font-weight: 700; letter-spacing: -1px; }
