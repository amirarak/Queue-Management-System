export default {
  dept: { name: 'Окуу бөлүмү' },

  nav: {
    kiosk: 'Студенттер үчүн киоск',
    display: 'Электрондук такта',
    staff: 'Кызматкердин панели',
    analytics: 'Аналитика',
    staffMgmt: 'Башкаруу'
  },

  common: {
    loading: 'Жүктөлүүдө...',
    error: 'Ката',
    save: 'Сактоо',
    cancel: 'Жокко чыгаруу',
    close: 'Жабуу',
    logout: 'Чыгуу',
    back: 'Артка',
    yes: 'Ооба, чыгуу',
    no: 'Жокко чыгаруу',
    tryAgain: 'Кайра аракет кылуу',
    print: 'Басып чыгаруу',
    printing: 'Басып чыгарылууда...'
  },

  logout: {
    confirmTitle: 'Системадан чыгуу',
    confirmText: 'Чыньдан чыккыңыз келеби?'
  },

  login: {
    title: 'Системага кириш',
    subtitle: 'Ala-Too International University',
    email: 'Email',
    password: 'Сырсөз',
    passwordPlaceholder: 'Сырсөздү киргизиңиз',
    submit: 'Кириш',
    submitting: 'Кирүүдө...',
    contactAdmin: 'Кирүү үчүн администраторго кайрылыңыз',
    emailRequired: 'Email талап кылынат',
    emailInvalid: 'alatoo.edu.kg почтасын колдонуңуз',
    passwordRequired: 'Сырсөз талап кылынат',
    forgotPassword: 'Сырсөздү унуттуңузбу?',
    forgotTitle: 'Сырсөздү унуттуңузбу?',
    forgotText: 'Сырсөздөр электрондук почта аркылуу орнотулат. Сырсөздү калыбына келтирүү үчүн администраторго кайрылыңыз.',
    forgotContact: 'Система администраторуна кайрылыңыз',
    forgotClose: 'Түшүндүм',
    forgotPassword: 'Сырсөздү унуттуңузбу?'
  },

  kiosk: {
    welcome: 'Кош келиңиздер!',
    instruction: 'Кезек номерин алуу үчүн экранды басыңыз',
    start: 'БАШТОО',
    selectService: 'Кызматты тандаңыз',
    inQueue: 'КЕЗЕКТЕ',
    creatingTicket: 'Талон түзүлүүдө...',
    queueError: 'Кезекти жүктөөдө ката',
    selectDepartment: 'Факультетиңизди тандаңыз'
  },

  services: {
    certificate: 'Окуу жөнүндө маалымкат',
    academicLeave: 'Академиялык өргүү',
    transfer: 'Башка адистикке которуу',
    curriculumChange: 'Окуу планын өзгөртүү',
    consultation: 'Консультация',
    other: 'Башка суроолор'
  },

  ticket: {
    ticket: 'Талон',
    yourNumber: 'Сиздин номериңиз',
    waiting: 'Чакырууну күтүңүз',
    minutes: 'мин'
  },

  display: {
    title: 'Электрондук кезек',
    serving: 'Тейленүүдө',
    ticketNumber: 'Талон номери',
    waiting: 'Чакырууну күтүү',
    recentlyCalled: 'Жакында чакырылгандар',
    queue: 'Кезек',
    next: 'Кийинки',
    emptyQueue: 'Кезек бош',
    window: 'Терезе'
  },

  staff: {
    title: 'Башкаруу панели',
    callNext: 'Кийинкини чакыруу',
    complete: 'Аяктоо',
    waiting: 'Күтүүдө',
    served: 'Тейленди',
    currentClient: 'Учурдагы кардар',
    noClient: 'Активдүү кардар жок',
    staffRole: 'Кызматкер',
    adminRole: 'Администратор',
    createSuccess: 'Кызматкер түзүлдү! Нускама каты жөнөтүлдү.',
    createError: 'Кызматкер түзүүдө ката',
    skip: 'Өткөрүп жибер',
    skipTitle: 'Талонду өткөрүп жибер',
    skipConfirmText: 'Кардар күтпөдүбү? Талон өткөрүп жиберилди деп белгиленет.'
  },

  staffMgmt: {
    title: 'Кызматкерлерди башкаруу',
    addStaff: 'Кызматкер кошуу',
    editStaff: 'Кызматкерди өзгөртүү',
    fullName: 'Толук аты-жөнү',
    emailLabel: 'Email',
    role: 'Ролу',
    status: 'Статусу',
    lastLogin: 'Акыркы кириш',
    actions: 'Аракеттер',
    active: 'Активдүү',
    inactive: 'Деактивдештирилген',
    edit: 'Өзгөртүү',
    deactivate: 'Деактивдештирүү',
    activate: 'Активдештирүү',
    delete: 'Жою',
    confirmDelete: 'Жоюуну ырастаңыз',
    confirmDeleteText: 'Бул кызматкерди жойгуңуз келеби',
    updateSuccess: 'Маалыматтар жаңыртылды!',
    logout: 'Чыгуу',
    emailNote: 'Кызматкер убактылуу сырсөзү бар кат алат',
    department:   'Факультет',
    noDepartment: 'Факультети жок',
    emailNote:    'Кызматкер убактылуу сырсөзү бар кат алат',
    emailLabel:   'Корпоративдик почта',
    duplicateEmail: 'Мындай email менен колдонуучу мурунтан эле бар',
    logoutConfirmText: 'Чындап эле тутумдан чыккыңыз келеби?',
    windowLabel:       'Терезе номери',
    windowPlaceholder: 'Мисалы: 1',
    windowNote:        'Чакырылганда студент кайсы терезеге барары'
  },

  analytics: {
    title: 'Аналитика жана статистика',
    export: 'Отчетту экспорттоо',
    period: 'Мезгил',
    today: 'Бүгүн',
    week: 'Жума',
    month: 'Ай',
    custom: 'Күндөрдү тандоо',
    from: 'Баштап',
    to: 'Чейин',
    apply: 'Колдонуу',
    totalTickets: 'Жалпы талондор',
    served: 'Тейленди',
    avgWait: 'Орточо күтүү убактысы',
    avgService: 'Орточо тейлөө убактысы',
    peakHour: 'Чоку саат',
    tickets: 'талон',
    hourlyChart: 'Сааттар боюнча бөлүштүрүү',
    staffStats: 'Кызматкерлер боюнча статистика',
    staffName: 'Кызматкер',
    ticketsServed: 'Тейленди',
    avgServiceShort: 'Орт. убакыт',
    topServices: 'Популярдуу кызматтар',
    noData: 'Бул мезгил үчүн маалымат жок',
    sec: 'сек',
    min: 'мин',
    deptStats: 'Факультеттер боюнча статистика',
    faculty: 'Факультет',
    completionRate: 'Аткарылды',
    skipped: 'Өткөрүп жиберилди'
  },

  profile: {
    title:                    'Менин профилим',
    back:                     'Артка',
    personalData:             'Жеке маалымат',
    fullName:                 'Толук аты-жөнү',
    email:                    'Корпоративдик почта',
    emailHint:                'Emailди өзгөртүүгө болбойт',
    saveChanges:              'Өзгөртүүлөрдү сактоо',
    saving:                   'Сакталууда...',
    saveSuccess:              'Маалымат ийгиликтүү жаңыртылды!',
    saveError:                'Сактоодо ката кетти',
    nameMinLength:            'Минимум 3 символ',
    daysInSystem:             'күн системада',
    changePassword:           'Сырсөздү өзгөртүү',
    currentPassword:          'Учурдагы сырсөз',
    currentPasswordPlaceholder: 'Учурдагы сырсөздү киргизиңиз',
    newPassword:              'Жаңы сырсөз',
    newPasswordPlaceholder:   'Минимум 8 символ',
    confirmPassword:          'Сырсөздү ырастаңыз',
    confirmPasswordPlaceholder: 'Жаңы сырсөздү кайталаңыз',
    changePasswordBtn:        'Сырсөздү өзгөртүү',
    passwordSuccess:          'Сырсөз ийгиликтүү өзгөртүлдү!',
    passwordError:            'Сырсөздү өзгөртүүдө ката',
    passwordsNoMatch:         'Сырсөздөр дал келбейт',
    minLength:                'Минимум 8 символ',
    strengthWeak:             'Алсыз',
    strengthFair:             'Орточо',
    strengthGood:             'Жакшы',
    strengthStrong:           'Күчтүү',
    activity:                 'Активдүүлүк',
    lastLogin:                'Акыркы кириш',
    status:                   'Статус',
    statusActive:             'Активдүү',
    roleLabel:                'Ролу',
    createdAt:                'Катталган күнү',
  },
  departments: {
    engineering: 'Инженерия жана информатика факультети',
    economics:   'Экономика жана башкаруу факультети',
    social:      'Социалдык илимдер факультети',
    medicine:    'Медицина факультети',
    humanities:  'Гуманитардык илимдер факультети',
  },
   forgotPassword: {
    title:       'Сырсөздү унуттуңузбу?',
    description: 'Корпоративдик почтаңызды киргизиңиз — ырастоо кодун жөнөтөбүз.',
    emailRequired: 'Email киргизиңиз',
    emailDomainOnly: '@alatoo.edu.kg даректери гана уруксат',
    accountNotFound: 'Мындай email менен аккаунт табылган жок',
    accountInactive: 'Аккаунт өчүрүлгөн. Администраторго кайрылыңыз',
    submit:      'Кодду жөнөтүү',
    verifyTitle: 'Ырастоо кодун киргизиңиз',
    verifyDesc:  'Код төмөнкү emailге жөнөтүлдү:',
    codeLabel:   'Каттагы код',
    newPasswordLabel: 'Жаңы сырсөз',
    newPasswordPlaceholder: 'Кеминде 8 белги',
    confirmPasswordLabel: 'Сырсөздү кайталаңыз',
    confirmPasswordPlaceholder: 'Сырсөздү дагы бир жолу киргизиңиз',
    resetSubmit: 'Сырсөздү жаңыртуу',
    resendCode:  'Кодду кайра жөнөтүү',
    invalidCode: 'Код туура эмес',
    passwordTooShort: 'Сырсөз кеминде 8 белгиден турушу керек',
    passwordPolicy: 'Сырсөздө чоң тамга, кичине тамга жана сан болушу керек',
    passwordsNoMatch: 'Сырсөздөр дал келген жок',
    successTitle: 'Сырсөз өзгөртүлдү!',
    successDesc:  'Эми жаңы сырсөз менен кире аласыз.',
    backToLogin: 'Кирүүгө кайтуу',
    linkText:    'Сырсөздү унуттуңузбу?',
  },
  setPassword: {
    checking: 'Шилтемени текшерүүдө...',
    invalidTitle: 'Шилтеме жараксыз',
    invalidText: 'Бул шилтеме мурунтан эле колдонулган же мөөнөтү өткөн. Администраторго кайрылыңыз.',
    title: 'Сырсөздү орнотуңуз',
    subtitle: 'Системага кируу үчүн сырсөз ойлоп табыңыз',
    newPassword: 'Жаңы сырсөз',
    placeholder: 'Минимум 8 символ',
    confirmPassword: 'Сырсөздү кайталаңыз',
    confirmPlaceholder: 'Сырсөздү дагы бир жолу киргизиңиз',
    req8chars: 'Минимум 8 символ',
    reqUppercase: 'Баш тамга',
    reqNumber: 'Цифра',
    submit: 'Сырсөздү орнотуу',
    successTitle: 'Сырсөз орнотулду!',
    successText: 'Эми системага кире аласыз.',
    goToLogin: 'Кирүүгө өтүү',
  }


}